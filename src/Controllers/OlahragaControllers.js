...
async function OlahragaControllers(res, url) {
  try {
    ...
    db.query(
      ...
      (err, results) => {
        if (err) {
          ...
          console.log("Berita Olahraga berhasil disimpan ke database Solopos");
        }
      }
    );
      // Simpan berita ke dalam database Berita_Olahraga
      const insertQueryBeritaOlahraga = `
      INSERT IGNORE INTO Berita_Olahraga(imgSrc, category, title, link, description, date, author, sentiment)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE
        imgSrc = VALUES(imgSrc),
        category = VALUES(category),
    ...
    db.query(
      ...
      (err, results) => {
        if (err) {
          ...
          console.log("Berita Olahraga berhasil disimpan ke database Olahraga");
        }
      }
    );
...
module.exports = OlahragaControllers;
